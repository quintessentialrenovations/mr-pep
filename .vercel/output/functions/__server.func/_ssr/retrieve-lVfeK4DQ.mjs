...
function norm(s) {
	return (s || "").toLowerCase().replace(/[’']/g, "").replace(/[–—]/g, "-").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9+.\s-]/g, " ").replace(/\s+/g, " ").trim();
}
function compactPhrase(s) {
	return norm(s).split(" ").filter((t) => t.length > 1 && !STOP.has(t)).join(" ");
}
function hasKw(n, k) {
	const key = norm(k);
	if (!key) return false;
...
function findHardStop(q) {
	const n = norm(q);
	for (const s of kb.hard_stops) if (s.triggers.some((t) => n.includes(norm(t)))) return s;
...
var GENERIC_TOK = /* @__PURE__ */ new Set([
	"peptide",
	"peptides",
	"copper",
	"skin",
...
var SHORT_NAMES = /* @__PURE__ */ new Set([
	"bpc",
	"kpv",
	"ghk",
	"hcg",
...
function findPeptides(q) {
	const n = ` ${norm(q)} `;
	const hits = [];
	for (const p of kb.peptides) if (distinctiveKeys(p).some((k) => {
		if (k.length <= 5) return n.includes(` ${k} `);
		return n.includes(k);
	})) hits.push(p);
	return [...new Map(hits.map((p) => [p.id, p])).values()];
}
function matchGoals(q) {
	const n = norm(q);
	const c = compactPhrase(q);
	return GOALS.filter((g) => g.keywords.some((k) => keywordHit(n, c, k)) || signalHit(n, g.signals)).sort((a, b) => GOAL_PRIORITY.indexOf(a.id) - GOAL_PRIORITY.indexOf(b.id));
}
function intent(q) {
	const n = norm(q);
	if (matchGoals(q).length) return "goal";
	const scores = {};
	for (const it of kb.intents) scores[it.id] = it.examples.reduce((a, e) => a + (n.includes(norm(e)) ? 1 : 0), 0);
...
function peptidesFromGoals(goals, cap) {
	const seen = /* @__PURE__ */ new Set();
	const out = [];
	const maxLen = Math.max(0, ...goals.map((g) => g.peptideIds.length));
	for (let i = 0; i < maxLen; i++) for (const g of goals) {
...
function combinedGoal(goals) {
	...
	return {
		...
		hint: {
			en: "Sequence, don't stack",
			es: "Secuencia, no apilar"
		},
...
function retrieve(q) {
	...
	if (goals.length >= 2) items.push({
		kind: "goal",
		card: combinedGoal(goals)
	});
	else if (goals.length === 1) items.push({
		kind: "goal",
		card: goals[0]
	});
	if (/tb-?500/.test(n) && /bpc/.test(n) && (/vs|versus|diferenc|compare| o | or /.test(n) || it === "compare")) items.push({
		kind: "compare",
		id: "bpc-tb"
	});
	...
	for (const p of peptides) items.push({
		kind: "peptide",
		card: p
	});
	const stackIds = new Set(goals.flatMap((g) => g.stackIds));
	const protocolIds = new Set(goals.flatMap((g) => g.protocolIds));
	if (/longevity|anti-?aging|longevidad/.test(n) && !goals.length) for (const s of kb.stacks.filter((s) => s.category === "longevity" || s.category === "mito")) items.push({
		kind: "stack",
		card: s
	});
	else if (/mito|mitochond|mots|ss-?31|nad\b|humanin|cardiolipin/.test(n) && !goals.length) for (const s of kb.stacks.filter((s) => s.category === "mito")) items.push({
		kind: "stack",
		card: s
	});
	else if ((/stack|combin|junto/.test(n) || it === "stack") && !peptides.length && !goals.length) for (const s of kb.stacks) items.push({
		kind: "stack",
		card: s
	});
	let stackCount = 0;
	...
	if (/injur|tendon|post-?op|post op|surgery|rehab|lesion|ligament/.test(n) && !goals.length) {
		for (const p of kb.protocols.filter((p) => /bpc|tb|injur|surg|tendon/i.test(p.id + p.name))) items.push({
			kind: "protocol",
			card: p
		});
		for (const s of kb.stacks.filter((s) => s.category === "injury")) items.push({
			kind: "stack",
			card: s
		});
	}
	if (/gut|ibd|leaky|intestino|sibo|gastr/.test(n) && !goals.some((g) => g.id === "gut")) {
		for (const p of kb.protocols.filter((p) => /gut|ibd|kpv/i.test(p.id + p.name))) items.push({
			kind: "protocol",
			card: p
		});
		for (const s of kb.stacks.filter((s) => s.category === "gut")) items.push({
			kind: "stack",
			card: s
		});
	}
	if (/safety|side effect|contraindic|who should not|warning|peligro|embaraz/.test(n) || it === "safety") items.push({ kind: "safety" });
	if (/cycle|washout|descanso|ciclo/.test(n) && !peptides.length && !goals.length) items.push({ kind: "safety" });
	const uniq = [];
	const seen = /* @__PURE__ */ new Set();
	for (const itm of items) {
		const k = itm.kind === "peptide" ? `p:${itm.card.id}` : itm.kind === "stack" ? `s:${itm.card.id}` : itm.kind === "protocol" ? `r:${itm.card.id}` : itm.kind === "compare" ? `c:${itm.id}` : itm.kind === "goal" ? `g:${itm.card.id}` : itm.kind;
...
function composeClarify(goals) {
	...
	const questions = qs.length ? qs : ["How long has this been going on?", "Are you already on a GLP-1, a peptide, or another medicine for this?"];
	return [
		`I heard: ${titles}.`,
...
function shouldClarify(goals, assistantTurns) {
...
function retrieveAsk(question, priorUser = []) {
...
function formatContext(items, hardStop, goals, map) {
	const bits = [];
	...
	for (const it of items) if (it.kind === "goal") {
		const g = it.card;
		bits.push([
			`GOAL ${g.title}`,
			g.plain,
...
//#endregion
export { shouldClarify as a, retrieveAsk as i, emptyInvite as n, matchGoals as r, composeClarify as t };
